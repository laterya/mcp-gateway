 ...
 *
 * <p>链路：SessionRootNode → SessionVerifyNode → StreamableCreateSessionNode → StreamableInitResponseNode
 */
@Slf4j
...
public class McpStreamableSessionService implements IMcpStreamableSessionService {
    ...
    private AbstractChainNode<SessionChainContext, Void> chain;

    @PostConstruct
    public void initChain() {
        var rootNode = new SessionRootNode<Void>("Streamable HTTP");
        var verifyNode = new SessionVerifyNode<Void>(authLicenseService);
        ...
        log.info("MCP Streamable HTTP 会话编排链初始化完成: SessionRootNode → SessionVerifyNode → StreamableCreateSessionNode → StreamableInitResponseNode");
    }
